...
module.exports = function(db){
   ...
   userModel.agregaNuevo = (email, password, handler)=>{
      var newUser = Object.assign({}, {email:email, password: genPassword(password), dateCreated: new Date().getTime(), active: true, lastPasswords:[], roles: ["public"]})
      userColl.insertOne(newUser, (err, result)=>{
        ...
        if(result.insertedCount == 0){
            return handler(new Error("No se guardó el usuario."), null);
        }
   ...
   userModel.changePassword = (email, newPassword, handler) => {
      ...
      userColl.findOne(query, {"projection": projection}, (err, user)=>{
         ...
         var lastPasswords = user.lastPasswords.slice(1,3);
         lastPasswords.push(user.password);
         var update = {
             "$ser": {"password": newPasswordHash, "lastPasswords": lastPasswords, "lastChangedPassword": new Date().getTime()}
         }
   ...
   function genPassword(rawPassword){
      var hashedPassword = bcrypt.hashSync(rawPassword, 10); //Cantidad de veces para recombinar en desencriptación. (< de 8 fácil, > de 12 lento)
